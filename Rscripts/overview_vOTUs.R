...
#####
  ...
  ### bacterial species
  bacterial_spec = read_tsv("Table_S6_microbial_species_overview.tsv") %>% mutate(Class = str_extract(gtdb_classification, "c__[^;]+") %>% str_remove("^c__") ) %>%
  ...
  # studies in UHGG with at least 1160 species
  UHGG_meta_studies = read_tsv("genomes-all_metadata_UHGG.tsv") %>% 
    dplyr::count(Study_accession) %>% filter(n > 1160)
  ...
  UHGG_1160 = read_tsv("genomes-all_metadata_UHGG.tsv") %>% semi_join(UHGG_meta_studies, by = "Study_accession") %>% mutate(Class = str_extract(Lineage, "c__[^;]+") %>% str_remove("^c__") ) %>%
    ...
    bind_rows(read_tsv("Table_S6_microbial_species_overview.tsv") %>% mutate(Class = str_extract(gtdb_classification, "c__[^;]+") %>% str_remove("^c__") ) %>%
